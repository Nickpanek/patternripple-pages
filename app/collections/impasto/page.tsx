// app/collections/impasto/page.tsx
"use client";
...
export default function ImpastoCollectionPage() {
  const [hoveredCard, setHoveredCard] = useState<string | null>(null);

  // Handles category as string or string[]
  const impastoProducts = useMemo(() => filterByCollection(products, "impasto"), []);
  const availableCount = useMemo(() => countAvailable(products, "impasto"), []);

  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-50 to-amber-100">
      <header className="bg-white/80 backdrop-blur-sm border-b border-stone-200 py-16">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <p className="text-lg text-stone-600">
            Painterly texture patterns with heavy brush build and sculpted surfaces
          </p>
        </div>
      </header>
      ...
      <div className="bg-gradient-to-r from-amber-700 to-stone-800 text-white py-3">
        <div className="max-w-6xl mx-auto px-4 text-center text-sm">
          {impastoProducts.length === 0
            ? "No Impasto Patterns Available"
            : `${availableCount} Exclusive Impasto Patterns Available`}
        </div>
      </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        ...
        {impastoProducts.length === 0 ? (
          <div className="text-center text-stone-600 py-24">
            <p>No impasto patterns available right now.</p>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {impastoProducts.map((product) => (
              <article
                key={product.sku}
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <p className="text-stone-600 text-sm mb-4">{product.subtitle}</p>
