// app/collections/seasonal/page.tsx
"use client";
...
export default function SeasonalCollectionPage() {
  ...
  const seasonalProducts = useMemo(() => filterByCollection(products, "seasonal"), []);
  const availableCount = useMemo(() => countAvailable(products, "seasonal"), []);
      ...
      <div className="bg-gradient-to-r from-green-500 to-teal-600 text-white py-3">
        <div className="max-w-6xl mx-auto px-4 text-center text-sm">
          {seasonalProducts.length === 0
            ? "No Seasonal Patterns Available"
            : `${availableCount} Exclusive Seasonal Patterns Available`}
        </div>
      </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-12">
        ...
        {seasonalProducts.length === 0 ? (
          <div className="text-center text-gray-600 py-24">
            <p>No seasonal patterns available right now.</p>
          </div>
        ) : (
